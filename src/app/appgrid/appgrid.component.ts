...
import {
  NgModule, Component, ViewEncapsulation, ElementRef, DoCheck, AfterContentInit, AfterViewInit, AfterViewChecked, OnInit, OnDestroy, Input, ViewContainerRef, ViewChild, IterableDiffers,
  Output, SimpleChange, EventEmitter, ContentChild, ContentChildren, Renderer2, QueryList, TemplateRef, ChangeDetectorRef, Inject, forwardRef, EmbeddedViewRef
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms'
import { SharedModule } from 'primeng/primeng';
import { PaginatorModule, CalendarModule } from 'primeng/primeng';
import { Column, Header, Footer, HeaderColumnGroup, FooterColumnGroup, PrimeTemplate, ButtonModule, MultiSelectModule } from 'primeng/primeng';
...
/**
Extending the p-column to implement the type of filter
*/
@Component({
  selector: 'app-column',
  template: ``,
...
export class CAMColumn extends Column {
  /**
  To toggling the filter visible
  */
  filterVisibile = false;

  /**
  To set filter colmn type
  */
  @Input() columnType = 'string';
  /**
  To set thedate format for the filter type date
  */
  svgPath = false;
  @Input() filterDateFormat = "dd/mm/yy"
  constructor() {
    super();
  }
}
  ...
  template: `
        <ng-template ngFor let-col [ngForOf]="columns" let-lastCol="last">
            <th #headerCell [ngStyle]="col.style" [class]="col.styleClass" [style.display]="col.hidden ? 'none' : 'table-cell'" (click)="dt.sort($event,col)" [attr.colspan]="col.colspan" [attr.rowspan]="col.rowspan"
                ...
                <span class="ui-sortable-column-icon ui-filter-column" *ngIf="col.filter" (click)='col.filterVisibile=true;filterClick($event,col)' [ngClass]="col.filterColor">
                ...
                <div style="width:100%"><span class="filterclose" (click)='filterClose(col)'><i class="fa fa-close"></i></span></div>
                  <div style="padding:10px">Show items with value that 
                    <div *ngIf="col.columnType!='values'&&col.columnType!='date'">
                     ...
                     <div *ngIf="col.columnType=='values'">
                     <p-multiSelect [style]="{'width':'100% !important'}" [options]="filterMatchMode" [(ngModel)]="col.filterValue"  styleClass="ui-column-filter ui-inputtext ui-widget ui-state-default ui-corner-all" ></p-multiSelect>
                       
                    </div>
                     <div *ngIf="col.columnType=='date'">
                        <p-calendar selectOtherMonths="true" [(ngModel)]="col.filterValue" dateFormat="{{col.filterDateFormat}}" [showIcon]="true"></p-calendar>
                        <p-calendar selectOtherMonths="true" [(ngModel)]="col.filterValue1" dateFormat="{{col.filterDateFormat}}" [showIcon]="true"></p-calendar>
                    </div>
                  </div>
...
export class ColumnHeaders {
  ...
  constructor( @Inject(forwardRef(() => AppGridComponent)) public dt: AppGridComponent) { }

  @Input("pColumnHeaders") columns: CAMColumn[];
  /**
  ...
  filterClick(e, col) {
    ...
    if (col.columnType.trim().toLowerCase() === 'number') {
      this.filterMatchMode = [{ label: 'Less than', value: 'lessThan' },
      ...
      col.filterMatchMode = (!col.filterMatchMode) ? "lessThan" : col.filterMatchMode;
    }
    else if (col.columnType.trim().toLowerCase() === 'boolean') {
      this.filterMatchMode = [{ label: 'True', value: 'boolTrue' },
      { label: 'False', value: 'boolFalse' }]
      col.filterMatchMode = (!col.filterMatchMode) ? "boolTrue" : col.filterMatchMode;
    }
    else if (col.columnType.trim().toLowerCase() === 'values') {
      let values = [];
      ...
      for (let i = 0; i < values.length; i++) {
        this.filterMatchMode.push({ label: values[i], value: values[i] })
      }
      col.filterValue = (!col.filterValue || col.filterValue == "") ? [] : col.filterValue;
      col.filterMatchMode = "custom";
    }
    else if (col.columnType.trim().toLowerCase() === 'date') {
      col.filterMatchMode = "date";
    }
    else {
      this.filterMatchMode = [{ label: 'Starts With', value: 'startsWith' },
      { label: 'Contains', value: 'contains' },
      { label: 'Ends With', value: 'endsWith' },
      { label: 'Exact', value: 'equals' },
      { label: 'In', value: 'in' }];
    }
    col.filterLeft = (e.clientX - 250 < 10) ? e.clientX : e.clientX - 250;
    col.filterTop = e.clientY;
   ...
   */
  filterBtn(e, col, name) {
    col.filterValue = (name == 'clear') ? '' : col.filterValue;
    col.filterValue1 = (name == 'clear') ? '' : col.filterValue1;
    let filterValue;

    if ((col.filterMatchMode == 'inbetween') || (col.filterMatchMode == 'date')) {
      filterValue = [col.filterValue, col.filterValue1]
    }
    else if (col.filterMatchMode == 'boolTrue') {
      filterValue = (name == 'clear') ? '' : "true";
    } else if (col.filterMatchMode == 'boolFalse') {
      filterValue = (name == 'clear') ? '' : "false";
    }
    else {
      filterValue = col.filterValue;
    }
    this.dt.onFilterInputClick(e);
    this.dt.onFilterKeyup(filterValue, col.field, col.filterMatchMode);
    col.filterVisibile = false;
    if ((name !== 'clear') && (filterValue.length > 0)) {
      col.filterColor = '';
      col.svgPath = true;
    } else {
      col.filterColor = '';
...
// <tr #rowElement *ngIf="rowIndex==0 &&dt.groupField"
...
//                     [ngClass]="{'ui-datatable-even':even&&dt.rowGroupMode!='rowspan','ui-datatable-odd':odd&&dt.rowGroupMode!='rowspan','ui-state-highlight': dt.isSelected(rowData)}" style="font-size:0;visibility:hidden;">
  ...
  template: `
        <ng-template ngFor let-rowData [ngForOf]="dt.dataToRender" let-even="even" let-odd="odd" let-rowIndex="index" [ngForTrackBy]="dt.rowTrackBy">
            ...
            <tr #rowGroupElement class="ui-widget-header ui-rowgroup-header" 
                *ngIf="dt.groupField && dt.rowGroupMode=='subheader' && (rowIndex === 0||(dt.resolveFieldData(rowData,dt.groupField) !== dt.resolveFieldData(dt.dataToRender[rowIndex - 1], dt.groupField)))"
                (click)="dt.onRowGroupClick($event);dt.groupHeaderClick($event,rowData)" [ngStyle]="{'cursor': dt.sortableRowGroup ? 'pointer' : 'auto'}">
                <td [attr.colspan]="dt.visibleColumns().length">
                    <a href="#" *ngIf="dt.expandableRowGroups" (click)="dt.toggleRowGroup($event,rowData)">                    
                    <i class="fa fa-caret-down" *ngIf= "dt.isRowGroupExpanded(rowData)"></i>
                    <i class="fa fa-caret-right" *ngIf= "!dt.isRowGroupExpanded(rowData)"></i>
                         </a>
                    <span class="ui-rowgroup-header-name" *ngIf="dt.rowGroupHeaderTemplate">
                        <p-templateLoader [template]="dt.rowGroupHeaderTemplate" [data]="rowData"></p-templateLoader>
                    </span>
                    <span class="ui-rowgroup-header-name" *ngIf="!dt.rowGroupHeaderTemplate">
                       {{rowData[dt.groupField]}}
                       <span style="margin-left:5px">{{dt.groupCountValue(rowData[dt.groupField])}}</span>
                    </span>
                </td>
            </tr>
            <tr #rowElement *ngIf="!dt.expandableRowGroups||dt.isRowGroupExpanded(rowData)" [class]="dt.getRowStyleClass(rowData,rowIndex)"
                    (click)="dt.handleRowClick($event, rowData,rowIndex)" (dblclick)="dt.rowDblclick($event,rowData)" (contextmenu)="dt.onRowRightClick($event,rowData)" (touchend)="dt.handleRowTouchEnd($event)"
                    [ngClass]="{'ui-datatable-even':even&&dt.rowGroupMode!='rowspan','ui-datatable-odd':odd&&dt.rowGroupMode!='rowspan','ui-datatable-all':dt.groupField,'ui-state-highlight': dt.isSelected(rowData)}">
                <ng-template ngFor let-col [ngForOf]="columns" let-colIndex="index">
                    <td #cell *ngIf="!dt.rowGroupMode || (dt.rowGroupMode == 'subheader') ||
                        (dt.rowGroupMode=='rowspan' && ((dt.sortField==col.field && dt.rowGroupMetadata[dt.resolveFieldData(rowData,dt.sortField)].index == rowIndex) || (dt.sortField!=col.field)))"
                        [ngStyle]="col.style" [class]="col.styleClass" [style.display]="col.hidden ? 'none' : 'table-cell'"
                        ...
                        <a href="#" *ngIf="col.expander" (click)="dt.toggleRow(rowData,$event)">
                            <span class="ui-row-toggler fa fa-fw ui-clickable" [ngClass]="{'fa-caret-down':dt.isRowExpanded(rowData), 'fa-caret-right': !dt.isRowExpanded(rowData)}"></span>
                        </a>
...
export class ScrollableView implements AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  constructor( @Inject(forwardRef(() => AppGridComponent)) public dt: AppGridComponent, public domHandler: DomHandler, public el: ElementRef, public renderer: Renderer2) { }
...
export class AppGridComponent implements AfterViewChecked, AfterViewInit, AfterContentInit, OnInit, OnDestroy, BlockableUI {
  ...
  @Input() sortableRowGroup: boolean = false;
  ...
  @Output() onRowGroupHeaderClick: EventEmitter<any> = new EventEmitter();

  @ContentChildren(PrimeTemplate) templates: QueryList<PrimeTemplate>;

  @ContentChildren(CAMColumn) cols: QueryList<CAMColumn>;
  ...
  constructor(public el: ElementRef, public domHandler: DomHandler, public differs: IterableDiffers,
    public renderer: Renderer2, public changeDetector: ChangeDetectorRef, public objectUtils: ObjectUtils, private utilServ: UtilityService) {
  ...
  /**
  Getting and setting the group field
  */
  @Input() get groupField(): string {
    return this._groupField;
  };
  set groupField(val: string) {
    this._groupField = (val.trim() == "") ? undefined : val;

    if (this.value && this._groupField) {
      this.groupFieldSort();
    }
  };
  @Input() get value(): any[] {
   ...
   */
  groupFieldSort() {
    if ((this.groupField) && (this.value !== null)) {
      let group = this.groupField;
      this.value.sort(function (a, b) {
        let value1 = a[group], value2 = b[group], result;
    ...
    if (this.copied == false) {
      this.copyValue = this.utilServ.copy(this.value, this.copyValue);
    }
    this.copied = false;
  }
  ngDoCheck() {
  ...
  groupCountValue(value) {
    let datasource = this.filteredValue || this.value;
    let count = 0;
    for (let i = 0; i < datasource.length; i++) {
      if (datasource[i][this.groupField] === value) {
        count++;
      }
    }
    return count;
        ...
        if (this.sortOrder === 0) {
          ...
          if (!this.groupField) {
            ...
            this.value.sort((data1, data2) => {
              let value1 = this.resolveFieldData(data1, this.sortField);
              let value2 = this.resolveFieldData(data2, this.sortField);
              let group1 = this.resolveFieldData(data1, this.groupField);
              let group2 = this.resolveFieldData(data2, this.groupField);
              let result = null;
              if (group1 > group2) {
                result = 1 * this.sortOrder;
              }
  ...
  filterConstraints = {
    date(value, filter) {
      if (filter[0] === undefined || filter[0] === null || filter[1] === undefined || filter[1] === null || filter[0] === '' || filter[1] === '') {
        return true;
      }
      ...
      let value1, value2;
      value1 = filter[0] > filter[1] ? filter[1] : filter[0];
      value2 = filter[0] < filter[1] ? filter[1] : filter[0];

      return new Date(value) >= new Date(value1) && new Date(value) <= new Date(value2);
    },
    custom(value, filter) {
    ...
    boolFalse(value, filter): boolean {
      if (filter === undefined || filter === null) {
        return true;
      }
    ...
    inbetween(value, filter): boolean {
      ...
      if (filter[0] === undefined || filter[0] === null || filter[1] === undefined || filter[1] === null || filter[0].trim() === '' || filter[1].trim() === '') {
        return true;
      }
      if (value === undefined || value === null) {
        return false;
    ...
    lessThanEqualTo(value, filter): boolean {
      if (typeof value !== 'number') {
        return false;
      }
      if (filter === undefined || filter === null || filter.trim() === '') {
    ...
    greaterThanEqualTo(value, filter): boolean {
      if (typeof value !== 'number') {
        return false;
      }
      if (filter === undefined || filter === null || filter.trim() === '') {
  ...
  groupHeaderClick(event: Event, row: any): void {
    let index = this.findExpandedRowGroupIndex(row);
    let rowGroupField = this.resolveFieldData(row, this.groupField);
    let data = this.value.filter(function (ele) {
      return ele[this.groupField] === rowGroupField
    }.bind(this))
    this.onRowGroupHeaderClick.emit({
      originalEvent: event,
      group: rowGroupField,
      data: data
    });
...
@NgModule({
  imports: [CommonModule, CalendarModule, SharedModule, PaginatorModule, FormsModule, DropdownModule, ButtonModule, MultiSelectModule],
  exports: [AppGridComponent, SharedModule, CAMColumn],
  declarations: [AppGridComponent, DTRadioButton, DTCheckbox, ColumnHeaders, ColumnFooters, TableBody, ScrollableView, RowExpansionLoader, CAMColumn],
